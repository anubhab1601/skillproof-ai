...
const FILTER_OPTIONS = [
  ...
  { key: 'payout', label: 'Payments', icon: 'payments' },
  { key: 'offer', label: 'Offers', icon: 'send' },
  { key: 'system', label: 'System', icon: 'settings' },
];

const NOTIF_ICON_MAP = {
  new_submission: 'assignment_turned_in',
  candidate_joined: 'person_add',
  payout_credited: 'payments',
  task_closed: 'schedule',
  offer_accepted: 'send',
...
const getCategory = (type) => {
  if (['new_submission', 'candidate_joined', 'task_closed', 'deadline_reminder'].includes(type)) return 'task';
  if (['payout_credited'].includes(type)) return 'payout';
  if (['offer_accepted', 'offer_expired'].includes(type)) return 'offer';
  return 'system';
};
    ...
    <>
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          ...
          <Link href="/company/settings" className="px-4 py-2 border border-[#c5c5d7] rounded-lg text-sm font-semibold text-[#454655] hover:bg-[#f2f3ff] transition-colors flex items-center gap-1">
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          filtered.map(n => {
            ...
            const colorClass = cat === 'payout' ? 'bg-emerald-100 text-emerald-600' :
              cat === 'offer' ? 'bg-purple-100 text-purple-600' :
              cat === 'system' ? 'bg-amber-100 text-amber-600' :
              'bg-[#eaedff] text-[#0623bb]';
